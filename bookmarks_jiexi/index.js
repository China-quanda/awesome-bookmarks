...
const fileName = 'bookmarks_2026_3_4.html';
...
function parse(html) {
  ...
  function foo($dt) {
    ...
    if ($h3.length == 0) {
      ...
      // 返回该书签的名称和网址组成的对象
      return $a.length > 0 ? { "id": 831,"categoryId": 157,"title": $a.text(), "url": $a.attr('href'), "icon": $a.attr('icon')||"","desc": "" } : null;
      return $a.length > 0 ? {
        name: $a.text(),
        url: $a.attr('href'),
        img: $a.attr('icon') || "",
